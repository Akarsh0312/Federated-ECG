...
def calculate_rmssd(r_peaks, fs=360):
    """
    r_peaks : array of R-peak sample positions
...
def load_mit_db(DS, winL, winR, do_preprocess, maxRR, use_RR, norm_RR, compute_morph, db_path, reduced_DS, leads_flag):
    ...
    return features, labels, patient_num_beats,my_db
...
def load_signal(DS, winL, winR, do_preprocess):
    ...
    # -----------------------------------
    # Stress / Fatigue Estimation (RMSSD)
    # -----------------------------------
    all_rmssd = []

    for r in range(len(Original_R_poses)):
        rmssd = calculate_rmssd(Original_R_poses[r])
        if rmssd is not None:
            all_rmssd.append(rmssd)

    if len(all_rmssd) > 0:
        avg_rmssd = np.mean(all_rmssd)

        my_db.rmssd_value = avg_rmssd

        if avg_rmssd < 20:
            my_db.stress_status = "High Stress / Fatigue"
            my_db.stress_label = 1
        else:
            my_db.stress_status = "Normal Stress"
            my_db.stress_label = 0
    else:
        my_db.rmssd_value = None
        my_db.stress_status = "Insufficient Data"
        my_db.stress_label = -1    
